---
title: "pg hardstorage llm export-session"
description: CLI reference for the pg hardstorage llm export-session command.
tags:
  - cli
  - reference
---
...
```
pg_hardstorage llm export-session <session-id> [flags]
```

### Options

```
  -h, --help          help for export-session
      --out string    output path for the bundle .tar.gz (default: ./llm-session-<id>.tar.gz)
      --repo string   repository URL (required — that's where the audit chain lives)
```
