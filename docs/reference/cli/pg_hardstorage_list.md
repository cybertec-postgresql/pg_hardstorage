---
title: "pg hardstorage list"
description: CLI reference for the pg hardstorage list command.
tags:
  - cli
  - reference
---
...
list shows backups for a deployment, newest first by stop time.
...
```
pg_hardstorage list <deployment> [flags]
```
...
```
  -h, --help              help for list
      --include-deleted   surface tombstoned (soft-deleted) backups alongside live ones (each row marked tombstoned=true)
      --only-deleted      show ONLY tombstoned backups — undelete-candidates discovery (implies --include-deleted)
