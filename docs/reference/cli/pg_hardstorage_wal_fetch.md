...
```
      --expect-system-identifier restore   refuse any WAL segment whose manifest records a different pg_control system_identifier — belt-and-braces against a deployment name reused across clusters; restore_command generated by restore passes the seed backup's identifier automatically
