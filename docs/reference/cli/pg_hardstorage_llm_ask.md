---
title: "pg hardstorage llm ask"
description: CLI reference for the pg hardstorage llm ask command.
tags:
  - cli
  - reference
---
...
Send the question to the configured LLM provider and print the
...
```
pg_hardstorage llm ask <question> [flags]
```

### Options

```
      --endpoint string   provider endpoint override (e.g. http://127.0.0.1:11434 for Ollama)
  -h, --help              help for ask
      --model string      model id (provider-specific)
      --provider string   LLM provider (default: env $PG_HARDSTORAGE_LLM_PROVIDER then 'mock')
```
