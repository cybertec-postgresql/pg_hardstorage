---
title: "pg hardstorage audit append"
description: CLI reference for the pg hardstorage audit append command.
tags:
  - cli
  - reference
---
...
```
pg_hardstorage audit append <action> [flags]
```

### Options

```
      --actor string        operator principal (free-form; appears in the event for audit)
      --deployment string   deployment subject (optional)
  -h, --help                help for append
      --reason string       free-form reason; lands in body.reason
      --repo string         repository URL (required)
      --tenant string       tenant scope (default: deployment's tenant or 'default')
```
