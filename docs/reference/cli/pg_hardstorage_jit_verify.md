---
title: "pg hardstorage jit verify"
description: CLI reference for the pg hardstorage jit verify command.
tags:
  - cli
  - reference
---
...
Decodes + validates a JIT token: signature, expiration,
...
```
pg_hardstorage jit verify [flags]
```
...
```
  -h, --help               help for verify
      --operation string   operation to authorise; matched against scope (required)
      --repo string        repository URL (required)
      --tenant string      tenant context for the verification
      --token string       the encoded token (base64url; required)
```
