---
title: "pg hardstorage repair chunks"
description: CLI reference for the pg hardstorage repair chunks command.
tags:
  - cli
  - reference
---
...
```
pg_hardstorage repair chunks --orphans|--missing [flags]
```

### Options

```
      --apply                    with --orphans: actually delete the orphans (default: dry-run)
  -h, --help                     help for chunks
      --min-chunk-age duration   minimum age an unreferenced chunk must reach before --apply reaps it; defends an in-flight backup whose manifest hasn't committed yet (pass 0 to disable) (default 24h0m0s)
      --missing                  list manifests that reference chunks the storage doesn't have
      --orphans                  list chunks not referenced by any visible manifest
