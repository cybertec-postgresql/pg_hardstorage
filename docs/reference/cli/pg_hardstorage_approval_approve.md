---
title: "pg hardstorage approval approve"
description: CLI reference for the pg hardstorage approval approve command.
tags:
  - cli
  - reference
---
...
```
pg_hardstorage approval approve <request-id> [flags]
```

### Options

```
      --approver string   operator-readable approver ID (e.g. email; appears in the audit chain)
  -h, --help              help for approve
      --key string        path to the approver's ed25519 PRIVATE-key PEM (defaults to the local keyring's signing key)
      --reason string     free-form reason; appears in the approval record
