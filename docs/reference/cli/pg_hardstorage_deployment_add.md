---
title: "pg hardstorage deployment add"
description: CLI reference for the pg hardstorage deployment add command.
tags:
  - cli
  - reference
---
...
```
pg_hardstorage deployment add <name> [flags]
```

### Options

```
      --connection string        libpq connection string (required)
  -h, --help                     help for add
      --repo string              repository URL (required)
      --schedule-backup string   backup schedule expression (default "every 6h")
      --schedule-rotate string   rotate schedule expression (default "daily_at 04:00")
      --skip-probe               don't connect to PG to validate the connection
      --tenant string            tenant scope (default: default)
      --yes                      replace an existing deployment with the same name without confirmation
```
