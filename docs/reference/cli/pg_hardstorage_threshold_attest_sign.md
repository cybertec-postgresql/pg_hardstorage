---
title: "pg hardstorage threshold attest sign"
description: CLI reference for the pg hardstorage threshold attest sign command.
tags:
  - cli
  - reference
---
...
Sign an attestation as the local operator.  The first signer
...
```
pg_hardstorage threshold attest sign <kind> <id> [flags]
```

### Options

```
      --as string       sign as a specific roster member (defaults to the unique member matching the local key)
      --hash string     subject hash (SHA-256 hex of canonical bytes; required)
  -h, --help            help for sign
      --repo string     repository URL (required)
      --roster string   roster ID (required)
```
