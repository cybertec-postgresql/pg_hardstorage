---
title: "pg hardstorage threshold roster create"
description: CLI reference for the pg hardstorage threshold roster create command.
tags:
  - cli
  - reference
---
...
```
pg_hardstorage threshold roster create <id> [flags]
```

### Options

```
      --created-by string     creator identity (defaults to one of the local-key's roster-member signer IDs)
      --description string    human-readable description
  -h, --help                  help for create
      --member strings        member spec 'signer:base64-pubkey' (repeatable; mutually exclusive with --members-file)
      --members-file string   path to a JSON file with [{signer,public_key},...] (mutually exclusive with --member)
      --repo string           repository URL (required)
      --threshold int         quorum size k (required, 1..n)
```
