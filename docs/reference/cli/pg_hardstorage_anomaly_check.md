---
title: "pg hardstorage anomaly check"
description: CLI reference for the pg hardstorage anomaly check command.
tags:
  - cli
  - reference
---
...
```
pg_hardstorage anomaly check <deployment> [flags]
```

### Options

```
      --all               score every backup against its predecessors instead of just the latest
  -h, --help              help for check
      --min-samples int   refuse to score when fewer than N same-type priors are available (default 3)
      --repo string       repository URL (required)
      --threshold float   |z-score| above which a metric is flagged (default 3.0 = three-sigma) (default 3)
      --window int        rolling-window size — number of most-recent same-type priors used for the baseline (default 10)
```
