...
IMPORTANT: at end-of-archive this command exits 6, NOT 1, and if you
hand-write a restore_command you must translate the codes yourself.
PostgreSQL hears only two things from restore_command: a plain nonzero
exit means "file not available" — which during unbounded recovery
means END OF ARCHIVE: stop, promote, report success — and a
termination by signal aborts recovery. So (1) map 6 -> 1, the honest
"no more WAL" answer, and (2) do NOT let any other failure exit
plainly: a storage outage or key failure that exits 1..255 reads as a
clean end-of-archive and PostgreSQL promotes a silently truncated
cluster. The line ` + "`pg_hardstorage restore`" + ` writes does both via the
shim '...; ec=$?; [ $ec = 0 ] && exit 0; [ $ec = 6 ] && exit 1;
kill -s ABRT $$' (one-shot recovery; standby restore_commands keep the
lenient '[ $ec = 6 ] && exit 1 || exit $ec' because a standby polls
forever and nonzero legitimately means "not archived yet").`,
	...
	c.Flags().StringVar(&repoURL, "repo", "",
		"repository URL (file://, s3://, ...) — must already exist (required)")
	_ = c.MarkFlagRequired("repo")
	c.Flags().StringVar(&expectSysID, "expect-system-identifier", "",
		"refuse any WAL segment whose manifest records a different pg_control system_identifier — belt-and-braces against a deployment name reused across clusters; restore_command generated by `restore` passes the seed backup's identifier automatically")
	return c
}
...
type walFetchOptions struct {
	...
	targetPath  string
	repoURL     string
	// expectSysID, when non-empty, refuses segments recorded under a
	// different cluster identity — see checkFetchSystemIdentifier.
	expectSysID string
}
...
func runWalFetch(cmd *cobra.Command, opts walFetchOptions) error {
	...
	if err := checkFetchSystemIdentifier(opts.expectSysID, m.SystemIdentifier, opts.segmentName); err != nil {
		return err
	}
...
type walStreamOptions struct {
	...
	// allowStandbySource permits streaming from a server in recovery.
	// Off by default: see guardSourceIsPrimary.
	allowStandbySource bool
...
func streamAttempt(
	...
	// preflight lints configuration and can be waived with
	// --skip-preflight, while this is a correctness guard about WHICH
	// SERVER we are talking to, and waiving it is exactly how the
	// silent case returns.
	if err := guardSourceIsPrimary(repoCtx, d, opts); err != nil {
	...
	if resumeNote == "fresh-slot-restart-lsn" {
