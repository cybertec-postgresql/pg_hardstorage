...
func TestAgentProcessBackupRestoreRoundtrip(t *testing.T) {
	...
	target := filepath.Join(t.TempDir(), "restored_agent")
	...
	if rres.FileCount == 0 {
		t.Error("agent process backup: zero file count")
	}
	...
	t.Logf("agent process backup/restore: %d files, %d bytes", rres.FileCount, rres.BytesWritten)
}
